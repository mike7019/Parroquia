# Configuracion de Parroquias y Sectores - CON MUNICIPIO Y PARROQUIA
Write-Host "=== CONFIGURACION COMPLETA CON RELACIONES ===" -ForegroundColor Cyan
...
# 5. Crear sectores para cada parroquia (AMBOS: parroquia_id y municipio_id)
    ...
    foreach ($sectorNombre in $sectoresNombres) {
        try {
            $sectorBody = @{
                ...
                municipio_id = $municipioId  # También incluir municipio
            } | ConvertTo-Json
...
try {
    ...
    foreach ($s in $finalSectores.data) {
        Write-Host "  - $($s.nombre) (ID: $($s.id_sector)) - Parroquia: $($s.parroquia.nombre)" -ForegroundColor Gray
    }
    
    Write-Host "`nESTRUCTURA JERARQUICA COMPLETA:" -ForegroundColor Cyan
