...
$result = Test-Endpoint -Method "POST" -Url "$baseUrl/api/auth/register" -Body $registerData -Description "User Registration"
$testResults += @{ Endpoint = "POST /api/auth/register"; Result = $result.Status }

# Verify the user in database for testing
if ($result.Status -eq "SUCCESS") {
    try {
        $verifyCmd = 'docker exec parroquia-postgres psql -U parroquia_user -d parroquia_db -c "UPDATE usuarios SET email_verificado = true WHERE correo_electronico = ''endpoint.test@example.com'';"'
        Invoke-Expression $verifyCmd | Out-Null
        Write-Host "   📝 User email verified for testing" -ForegroundColor Gray
    } catch {
        Write-Host "   ⚠️ Could not verify user email" -ForegroundColor Yellow
    }
}
...
if ($global:accessToken) {
    $changePasswordData = @{
        currentPassword = "EndpointTest123!"
        newPassword = "NewEndpointTest123!"
    } | ConvertTo-Json
    ...
    # Change it back
    $revertPasswordData = @{
        currentPassword = "NewEndpointTest123!"
        newPassword = "EndpointTest123!"
    } | ConvertTo-Json
...
# Forgot Password
$forgotPasswordData = @{ email = "endpoint.test@example.com" } | ConvertTo-Json
...
$resendData = @{ email = "endpoint.test@example.com" } | ConvertTo-Json
...
# List of catalog endpoints to test
$catalogEndpoints = @(
    "parroquias",
    "veredas", 
    "municipios",
    ...
    "celebraciones-padre-madre",
    "celebraciones-familia",
    "comunidades-culturales",
    "liderazgos",
    "tipo-viviendas",
    "sistemas-acueducto",
    "tipos-aguas-residuales",
    "tipos-disposicion-basura",
    "talla-vestimenta"
)
...
foreach ($catalog in $catalogEndpoints) {
    ...
    # Search in catalog (if it supports search)
...
# Sector Schema
$result = Test-Endpoint -Method "GET" -Url "$baseUrl/sector-schema" -Description "Sector Schema"
$testResults += @{ Endpoint = "GET /sector-schema"; Result = $result.Status }
