  ...
  for (const miembro of familyMembers) {
    try {
      ...
      const personaData = {
        ...
        correo_electronico: miembro.correo_electronico || `${(miembro.nombres || 'temp').toLowerCase().replace(/\s+/g, '_')}.${Date.now()}.${personasCreadas}@temp.com`,
        ...
        if (personaExistente.length > 0) {
          ...
          await sequelize.query(`
            UPDATE personas SET
              ...
              id_estado_civil_estado_civil = :idEstadoCivil,
              estudios = :estudios,
          ...
          `, {
            replacements: {
              ...
              correo: miembro.correo_electronico || correoActual || null,
