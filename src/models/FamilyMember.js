...
const FamilyMember = sequelize.define('FamilyMember', {
    ...
    validate: {
      isValidTalla(value) {
        if (!value || typeof value !== 'object') {
          throw new Error('Talla debe ser un objeto válido');
        }
        if (!value.camisa && !value.pantalon && !value.calzado) {
          throw new Error('Al menos una talla debe ser especificada');
        }
      }
    }
  },
  ...
  enQueEresLider: {
    ...
    comment: 'Áreas de liderazgo'
  },
  habilidadDestreza: {
    type: DataTypes.STRING(300),
    allowNull: true,
    field: 'habilidad_destreza',
    comment: 'Habilidades y destrezas especiales'
  },
  correoElectronico: {
  ...
  profesionMotivoFechaCelebrar: {
    ...
    validate: {
      isValidCelebration(value) {
        if (!value || typeof value !== 'object') {
          throw new Error('Datos de celebración deben ser un objeto válido');
        }
      }
    }
  },
...
FamilyMember.addHook('beforeSave', (member) => {
  ...
  // Validar tipo de identificación según edad
  if (age < 7 && member.tipoIdentificacion !== 'RC') {
    throw new Error('Menores de 7 años deben tener Registro Civil');
  }
  
  if (age >= 7 && age < 18 && member.tipoIdentificacion !== 'TI') {
    throw new Error('Menores entre 7-17 años deben tener Tarjeta de Identidad');
  }
});
