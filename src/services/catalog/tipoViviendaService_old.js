...
class TipoViviendaService {
  ...
  async findNextAvailableId() {
    try {
      ...
      const existingRecords = await TipoVivienda.findAll({
        attributes: ['id_tipo_vivienda'],
        order: [['id_tipo_vivienda', 'ASC']],
        raw: true
      });
      ...
      const existingIds = existingRecords.map(record => parseInt(record.id_tipo_vivienda));
      ...
      logger.error('Error finding next available ID for tipo vivienda:', error);
  ...
  async createTipo(tipoData) {
    try {
      ...
      const nuevoTipo = await TipoVivienda.create({
        ...tipoData,
        id_tipo_vivienda: nextId
      });
