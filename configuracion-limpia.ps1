...
# Municipio Medellin
$municipioId = "753"
Write-Host "Usando Municipio: Medellin (ID: $municipioId)" -ForegroundColor Green

# Crear 4 parroquias
Write-Host "`n--- CREANDO PARROQUIAS ---" -ForegroundColor Yellow
$nombresParroquias = @("San Jose", "La Candelaria", "Cristo Rey", "San Antonio")
$parroquiasCreadas = @()
...
foreach ($parroquia in $parroquiasCreadas) {
    ...
    foreach ($sectorNombre in $nombresSectores) {
        try {
            ...
            Write-Host "  Error $sectorNombre - $($_.Exception.Message)" -ForegroundColor Red
        }
    }
}
...
try {
    ...
    Write-Host "Sectores creados en esta sesion: $sectoresCreados" -ForegroundColor Green
    
    Write-Host "`nCONFIGURACION COMPLETADA!" -ForegroundColor Green
    Write-Host "Jerarquia: Municipio -> Parroquia -> Sector" -ForegroundColor Green
