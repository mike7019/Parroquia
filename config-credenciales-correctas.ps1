# Configuracion de Parroquias y Sectores - Credenciales Correctas
...
# 2. Obtener municipio Abejorral
Write-Host "`n2. Obteniendo municipio..." -ForegroundColor Yellow
try {
    $municipios = Invoke-RestMethod -Uri "http://localhost:3000/api/catalog/municipios" -Headers $headers
    $abejorral = $municipios.datos | Where-Object { $_.nombre -eq "Abejorral" }
    if ($abejorral) {
        Write-Host "Municipio: $($abejorral.nombre) (ID: $($abejorral.id_municipio))" -ForegroundColor Green
        $municipioId = $abejorral.id_municipio
    } else {
        Write-Host "Municipio Abejorral no encontrado" -ForegroundColor Red
        exit 1
    }
} catch {
    Write-Host "Error obteniendo municipio: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    $primerosSectores = $finalSectores.datos | Select-Object -First 5
