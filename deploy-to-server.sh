...
# Versión: 2.1 - Optimizado para servidor de producción

echo "🚀 Iniciando deployment en servidor de producción..."
echo "📅 Fecha: $(date)"
echo "🔄 Versión script: 2.1"
...
APP_DIR="/var/www/parroquia" # Directorio típico en servidor
LOG_DIR="logs"
...
sync_production_database() {
    ...
    echo ""
    echo "¿Continuar con la sincronización? (escriba 'SI' para confirmar):"
    read confirmation
...
const verifyProductionFeatures = async () => {
  try {
    ...
    // 1. Verificar campo comunionEnCasa
    ...
    if (comunionField.length > 0) {
      console.log('✅ Campo comunionEnCasa configurado correctamente');
      console.log(`   Tipo: ${comunionField[0].data_type}, Default: ${comunionField[0].column_default}`);
    } else {
