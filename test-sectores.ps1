...
Write-Host "`n2️⃣ Getting municipios..." -ForegroundColor Green
...
Write-Host "`n3️⃣ Creating sectors..." -ForegroundColor Green
...
Write-Host "`n4️⃣ Getting all sectors..." -ForegroundColor Green
...
if ($allSectoresResponse) {
    if ($allSectoresResponse.data -and $allSectoresResponse.data.Count -gt 0) {
        ...
        Write-Host "⚠️ No sectors found in response" -ForegroundColor Yellow
    }
} else {
    Write-Host "❌ Failed to get sectors list" -ForegroundColor Red
}

# Step 5: Get sector by ID
if ($createdSectores.Count -gt 0) {
    Write-Host "`n5️⃣ Getting sector by ID..." -ForegroundColor Green
    ...
    if ($sectorByIdResponse) {
        Write-Host "✅ Sector found: $($sectorByIdResponse.nombre) - Municipio: $($sectorByIdResponse.municipio.nombre_municipio)" -ForegroundColor Green
    } else {
        Write-Host "❌ Failed to get sector by ID" -ForegroundColor Red
    }
}

# Step 6: Update a sector
if ($createdSectores.Count -gt 0) {
    Write-Host "`n6️⃣ Updating sector..." -ForegroundColor Green
    $sectorToUpdate = $createdSectores[0]
    $updateData = @{
        nombre = "Sector Central Actualizado"
    }
    
    $updateResponse = Invoke-AuthenticatedRequest -Method "PUT" -Uri "$apiUrl/catalog/sectores/$($sectorToUpdate.id_sector)" -Headers $headers -Body $updateData
    
    if ($updateResponse) {
        Write-Host "✅ Sector updated: $($updateResponse.nombre)" -ForegroundColor Green
    } else {
        Write-Host "❌ Failed to update sector" -ForegroundColor Red
    }
}

Write-Host "`n🎉 Sectores testing completed!" -ForegroundColor Cyan
...
Write-Host "- All sectors are now linked only to municipios (no parroquia dependency)" -ForegroundColor Green
