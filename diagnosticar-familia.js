...
import { Familias, sequelize } from './src/models/index.js';
...
async function diagnosticarProblemaFamilia() {
  try {
    ...
    const secuenciaQuery = `SELECT last_value, is_called FROM familias_id_familia_seq;`;
    ...
    const count = await Familias.count();
    ...
    try {
      const familiaTest = await Familias.create({
