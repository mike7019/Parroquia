...
async function testNameConcatenationFix() {
  try {
    ...
    // Función para formatear nombres completos (copiada del controller)
    function formatearNombreCompleto(primerNombre, segundoNombre, primerApellido, segundoApellido) {
    ...
    for (const nombreCompleto of casosNombres) {
      ...
      const nombreFinal = formatearNombreCompleto(parsed.primerNombre, parsed.segundoNombre, parsed.primerApellido, parsed.segundoApellido);
