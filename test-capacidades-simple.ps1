...
# Funcion para hacer requests autenticados
...
Write-Host "`n1. Obteniendo token de autenticacion..." -ForegroundColor Yellow
...
Write-Host "`n2. Probando endpoint de filtros disponibles..." -ForegroundColor Yellow
...
Write-Host "`n3. Probando consulta por destrezas..." -ForegroundColor Yellow
...
if ($destrezas) {
    ...
    Write-Host "   - Personas en pagina: $($destrezas.datos.Count)" -ForegroundColor Cyan
}

Write-Host "`n4. Probando analisis geografico..." -ForegroundColor Yellow
$geografia = Invoke-AuthenticatedRequest -Url "http://localhost:3000/api/personas/capacidades/analisis-geografico" -Token $token
if ($geografia) {
    Write-Host "Analisis geografico exitoso:" -ForegroundColor Green
...
Write-Host "`n5. Probando consulta por profesiones..." -ForegroundColor Yellow
...
if ($profesiones) {
    ...
    Write-Host "   - Estadisticas de profesiones: $($profesiones.estadisticas_profesiones.Count)" -ForegroundColor Cyan
}

Write-Host "`n6. Probando comunidades culturales..." -ForegroundColor Yellow
...
Write-Host "`nPara mas pruebas detalladas:" -ForegroundColor Yellow
Write-Host "   - Abre Swagger UI: http://localhost:3000/api-docs" -ForegroundColor Cyan
Write-Host "   - Busca la seccion Personas y Capacidades" -ForegroundColor Cyan
Write-Host "   - Usa el token Bearer en el boton Authorize" -ForegroundColor Cyan
